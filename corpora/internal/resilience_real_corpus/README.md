...
## Corpus evaluation layer

`corpus_runner.py` consumes JSONL records with explicit provenance (`fixture`, `real_patch`, `real_repository`, `synthetic_mutation`) and separates harness validation from real-corpus results. The example dataset is intentionally fixture-only and must not be called a production baseline.

Run:

```bash
python corpus_runner.py baseline/corpus-example.jsonl --output baseline/corpus-example-summary.json
```

See `ASSUMPTION_REVIEW.md` before promoting any metric to a production policy.
