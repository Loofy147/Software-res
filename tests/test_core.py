...
def test_all_schemas_load():
    for name in [
        ...
        schema=json.loads(resources.files('resilience_poc').joinpath('resources','schemas',name).read_text(encoding='utf-8'))
...
def test_missing_verification_evidence_cannot_become_pass():
    ...
    from resilience_poc.storage import get_json, put_json
    from resilience_poc.security import sign_json
    from resilience_poc.collector import PRIVATE
    from resilience_poc.validator import validate_manifest

    r = run_experiment("A")
    manifest = get_json(r["manifest_id"])
    patch = get_json(manifest["generated_patch_ref"])
    patch.pop("fixture_test_evidence", None)
    patch.pop("fixture_dependency_report", None)
    unsigned = dict(patch)
    unsigned.pop("integrity_signature", None)
    patch["integrity_signature"] = sign_json(unsigned, PRIVATE)
    put_json(patch, patch["id"])

    checked = validate_manifest(manifest["id"])
    assert checked["status"] == "review"
    assert checked["errors"] == ["MISSING_VERIFICATION_EVIDENCE"]
    assert checked["reliability_vector"]["decision"]["outcome"] == "REVIEW"
